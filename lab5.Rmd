...
pmean <- function(directory = 'specdata', pollutant, id = c(1:332)){
  ...
  for (i in 1:length(id)) {
    ...
    if (pollutant == 'sulfate') {
      u[i] <- mean(na.omit(as.numeric(df$sulfate)))
    } else if (pollutant == 'nitrate'){
      u[i] <- mean(na.omit(as.numeric(df$nitrate)))
